...
──────── IMPORTANT STYLE NOTES (mandatory) ────────
• Recipe must include ALL necessary steps - DO NOT limit the number of steps
...
• López-Alt style example: "Heat butter in a 10-inch skillet over medium-high heat until melted. Continue cooking, swirling pan constantly until butter is dark golden brown and has nutty aroma, about 1 minute. Remove skillet from heat and stir in capers, lemon juice, and parsley. Swirl to combine, creating an emulsified sauce."
...
• NO step limits - include as many detailed steps as needed for complete recipe
...
──────── RETURN JSON (schema fixed) ────────
{
  ...
  "calorie_check_pass": boolean
    ...
    const response = await openai.chat.completions.create({
      ...
      temperature: 0.7,
      max_tokens: 1500, // Increased from 700 to allow for more detailed steps
      messages: [
        {
          role: "system",
          content:
            "You are a culinary scientist in the López-Alt tradition creating detailed, scientifically-grounded recipes. Your strength is writing THOROUGH, DETAILED recipe steps that explain the science behind each technique. NEVER limit the number of steps - include ALL necessary instructions with scientific explanations. Follow all DATA PIPELINE, TONE & STEP, and SHOPPABLE rules exactly. Output RAW JSON only—no markdown, comments, or extra keys.",
        },
